...
waitForNetwork()
{
    ...
    do
    {
	 status="$([  -f  /tmp/route_available ] && echo "open" ||echo "closed")"
         if [ "$status" = "open" ] ; then
            log "connectivity check passed"
            # Kept for non-community builds so that they can use their app.
            residentApp=""
            appurl=$residentApp
            break;
         else
...
FireboltRefUI=$(tr181 -g Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.FireboltRefUI.Enable 2>&1)
if [ "${FireboltRefUI}" = "true" ];then
    touch /tmp/enableRipple
    log "FireboltRefUI RFC is true. Exit without launching UI using Thunder; Ripple will launch it."
    exit 0
fi
log "Selected reference app is $appurl"
RDKShellPluginStatus="deactivated"
getRDKShellPluginStatus()
{
    output=`curl -sS --header "Content-Type: application/json" -H "Authorization: Bearer $TOKEN" --request POST --data '{"jsonrpc": "2.0", "id": 1234567890, "method": "Controller.1.status@org.rdk.RDKShell"}' http://127.0.0.1:9998/jsonrpc`
    RDKShellPluginStatus=`echo $output | awk -F"[,:}]" '{for(i=1;i<=NF;i++){if($i~/'state'\042/){print $(i+1)}}}' | tr -d '"'`
}
getRDKShellPluginStatus
waitForRDKShellPluginUp()
{
    while [ "$RDKShellPluginStatus" != "activated" ]
    do
    {
        getRDKShellPluginStatus
        if [ "$RDKShellPluginStatus" = "activated" ] ; then
            log "RDKShell plugin activated"
            break;
        else
            log "RDKShell plugin not activated"
            sleep 1
        fi
    }
    done
